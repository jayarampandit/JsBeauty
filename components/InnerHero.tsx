...
export default function InnerHero({ eyebrow, title, copy, image, mobileImage, brightOverlay, noImageMobile }: { eyebrow: string; title: string; copy: string; image: string; mobileImage?: string; brightOverlay?: boolean; noImageMobile?: boolean }) {
  const classes = ["inner-hero", mobileImage && "inner-hero-split-wash", brightOverlay && "inner-hero-bright", noImageMobile && "inner-hero-no-image-mobile"].filter(Boolean).join(" ");
